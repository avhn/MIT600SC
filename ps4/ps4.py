...
def apply_coder(text, coder):
    ...
    'Hello, world!'
    """
    tester = string.uppercase + string.lowercase + ' '
    new_text = ''
    for char in text:
        if char in tester:
            char = coder[char]
...
def find_best_shift(wordlist, text):
    ...
    'Hello, world!'
    """
    result = (0, 0) ##(count, shift)
    for shift in range(1, 27):
        count = 0
        words = apply_coder(text, build_decoder(shift)).split()
...
def apply_shifts(text, shifts):
    ...
    for e in shifts:
        start, shift = e
        new_text = new_text[:start] + apply_shift(new_text[start:], shift)
...
def find_best_shifts(wordlist, text):
  ...
  **  returns: ENCODING keys in form of list of tuples.  each tuple is (position in text, amount of shift)
    ...
    for i in range(0,27):
        ...
        if space != -1:
            if is_word(wordlist, s[start:space]):
                recur = find_best_shifts_rec(wordlist, s, space+1)
                if recur != None and recur[-1] != None:
                    return [(start, i)] + recur
        else:
